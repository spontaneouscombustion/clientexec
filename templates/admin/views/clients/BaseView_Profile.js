    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.error) {
                ...
                msg(clientexec.lang("There was an error when updating your status"));
            } else if(data.message) {
                msg(data.message);
            } else {
                msg(clientexec.lang("Status has been updated successfully"));
    ...
    switch('' + newstatusAliasto){ // stupid dynamic languages
        ...
        case '1':
            if ((oldstatusAliasto < 1)&&(haspackages)) {
                hascallback = true;
                RichHTML.msgBox(clientexec.lang('Do you wish to activate this clients packages as well?'),
                {
                    type:"confirm"
                }, function(result) {
                    ...
                    if ( hasplugin ) {
                        RichHTML.msgBox(clientexec.lang('Do you wish to use the server plugin to activate these packages?'),
                        {
                            type:"confirm"
                        }, function(result) {
                            ...
                            } else if(result.btn == clientexec.lang('Cancel')) {
                                return;
                            }
                            UpdateStatus(returnarray);
                        });
        ...
        case '-1':
            if(haspackages){
                hascallback = true;
                RichHTML.msgBox(clientexec.lang('Do you wish to suspend this clients packages as well?'),
                {
                    type:"confirm"
                }, function(result) {
                    if ( result.btn == clientexec.lang('Cancel') ) {
                    ...
                    returnarray.packageaction=1;
                    if ( hasplugin ) {
                        RichHTML.msgBox(clientexec.lang('Do you wish to use the server plugin to suspend these packages?'),
                        {
                            type:"confirm"
                        }, function(result) {
                            if (result.btn == clientexec.lang('Yes')) {
        ...
        case '-3':
            if(haspackages){
                hascallback = true;
                RichHTML.msgBox(clientexec.lang('Do you wish to cancel this clients packages as well?'),
                {
                    type:"confirm"
                }, function(result) {
                    if ( result.btn == clientexec.lang('Cancel') ) {
                    ...
                    returnarray.packageaction=1;
                    if ( hasplugin ) {
                        RichHTML.msgBox(clientexec.lang('Do you wish to use the server plugin to cancel these packages?'),
                        {
                            type:"confirm"
                        }, function(result) {
                            if (result.btn == clientexec.lang('Yes')) {
...
$(document).ready(function(){
    ...
    $('.register-guest-btn-wrapper .btn').bind('click', function() {
        RichHTML.msgBox(clientexec.lang('Are you sure you wish to register this account?'), {
            type: 'confirm'
        }, function(result) {
            if (result.btn == clientexec.lang('No') || result.btn == clientexec.lang('Cancel')) {
                return;
            }
