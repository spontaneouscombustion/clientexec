...
ticketview.updateticket_view = function(metadata) {
    ...
    $('.ticket-time-elapsed').text(clientexec.lang('Time Elapsed: %', metadata.time_elapsed));

    if (metadata.last_reply != '0s') {
        $('.ticket-time-last-reply').text(clientexec.lang('Last Reply: %', metadata.last_reply));
    }
    //set status
