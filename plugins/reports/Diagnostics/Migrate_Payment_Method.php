...
class Migrate_Payment_Method extends Report
{
    ...
    function process()
    {
        ...
        echo "<div style='margin:20px;'><h3>" . $this->user->lang('Migrate Stripe CC Accounts to Stripe') . "</h3>";
        echo "<br/>";
        echo $this->user->lang('Passphrase').":&nbsp;&nbsp;<input type='password' name='passphraseStripe' id='passphraseStripe' style='width:100px;' >&nbsp&nbsp&nbsp&nbsp";
        echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button class='btn' type='button' data-loading-text='Loading...' onclick='MigrateToStripe(document.getElementById(\"passphraseStripe\").value);'>"
            .$this->user->lang("Migrate to Stripe") . "</button>&nbsp;&nbsp;&nbsp;";
        echo "<br/><br/>";
        echo "</div>";
        echo "\n\n<script type='text/javascript'>
                function MigrateToStripe(passphrase){
                    if(".(($this->user->hasPermission( 'billing_create' ))? "true" : "false")."){
                        if(passphrase == ''){
                            RichHTML.error(lang('Passphrase is required.'));
                        }else{
                            var optionsSelected = '\'stripe\'';
                            location.href='index.php?fuse=reports&view=viewreport&controller=index&report=Migrate+Payment+Method&type=Diagnostics&migrate=1&data='+btoa(btoa(passphrase)+'_'+btoa(optionsSelected)+'_'+btoa('stripe'));
                        }
                    }else{
                        RichHTML.error(lang('You do not have permission to perform this action.'));
                    }
